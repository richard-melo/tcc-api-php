...
if [[ -z "$RUN_ID" ]]; then
    echo "ERRO: timeout aguardando o run iniciar. Verifique as Actions no GitHub." >&2
    exit 1
fi
...
python3 - "$WORK_DIR/jobs.json" "$WORK_DIR/times.txt" << 'PY'
import json, sys
...
# ── 5. Cobertura e contagem de testes via artefato ────────────────────────────
echo ""
echo "[5/5] Baixando artefato de cobertura..."
COVERAGE="N/A"
TESTS_TOTAL="N/A"
TESTS_PASSED="N/A"
TESTS_FAILED="N/A"
...
if [[ -n "$ARTIFACT_ID" ]]; then
    ...
    if command -v gh &>/dev/null; then
        ...
        # Obtém URL de redirect do ZIP do artefato
        ...
        if [[ -n "$ARTIFACT_URL" ]]; then
            curl -sL "$ARTIFACT_URL" -o "$WORK_DIR/artifact.zip" 2>/dev/null
            unzip -q "$WORK_DIR/artifact.zip" -d "$WORK_DIR/artifact/" 2>/dev/null || true
        fi
    fi

    # Parse coverage.xml (formato Clover)
...
try:
    ...
    cov   = int(m.get('coveredstatements', 0))
    print(round(cov / stmts * 100, 1) if stmts > 0 else 'N/A')
...
" 2>/dev/null || echo "N/A")
    ...
    # Parse junit.xml para contagem de testes
...
" 2>/dev/null || echo "N/A N/A N/A")
    ...
    echo "      Artefato não encontrado (coverage N/A)"
fi
